...
public class Smartcard extends Applet implements ISO7816 {
	...
	// Protocol states
	public static final byte PROT_STATE_VEHICLE_STARTED = 0x01;

	// Messages
	public static final byte MSG_IGNITION = 0x45;
	public static final byte MSG_IGNITION_OK = 0x46;
	public static final byte MSG_DEDUCT_KM = 0x48;
	public static final byte MSG_DEDUCT_OK = 0x49;
	public static final byte MSG_NOT_ENOUGH_KM = 0x47;
	public static final byte MSG_STOP = 0x48;
	public static final byte MSG_STOP_OK = 0x49;
	public static final byte MSG_TOPUP = 0x4a;
	public static final byte MSG_TOPUP_OK = 0x4b;
	public static final byte MSG_REFUND_OK = 0x4c;
	...
	private void authenticate7(APDU apdu, byte[] buf) {
		apdu.setIncomingAndReceive();
		sd.sessionDecrypt(buf, OFFSET_CDATA, SecureData.SIZE_AES_BLOCKSIZE,
				tmp, (short) 0);
		sd.setSecretKey(tmp, (short) 0);
		setAuthenticated(true); // TODO: is this reset to false?
	}
	...
	private void startVehicle(APDU apdu, byte[] buf) {
		...
		apdu.setIncomingAndReceive();
		sd.sessionDecrypt(buf, (short) 0, SecureData.SIZE_AES_BLOCKSIZE, tmp,
				(short) 0);
		if (tmp[0] != MSG_IGNITION) {
		...
		if (km.start()) {
			...
			sd.sessionEncrypt(tmp, (short) 0, len, buf, (short) 0);
			// FIXME: len
	...
	/**
	 * Protocol 6.9 (renewing the certificate), step 4 and 5. Protocol 6.10/6.11
	 * (topup/refund), step 3
	 */
	private void renewCertificate(APDU apdu, byte[] buf) {
		// FIXME: in our protocol, this message is not encrypted with the
		// session key
		apdu.setIncomingAndReceive();
		sd.sessionDecrypt(buf, (short) 0, SecureData.SIZE_AES_BLOCKSIZE, tmp,
				(short) 0);
